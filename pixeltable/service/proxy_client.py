...
class R2PartSink(PartSink):
    ...
    def add_media_file(self, path: str) -> str:
        key = f'{self._key_prefix}{self._num_media_parts}{Path(path).suffix}'
        self._num_media_parts += 1
        url = f'pxtfs://{self._org}:{self._db}/home/{key}'
        self._get_store().copy_local_file(Path(path), FileDestination(url=url, remote_key=key))
        return key
...
class ProxyClient:
    ...
    # the hosted org/db this client talks to (None for a local daemon); determines how media parts travel
    _org: str | None
    _db: str | None

    def __init__(self, transport: Transport, org: str | None = None, db: str | None = None):
    ...
    def remote(cls, org: str, db: str, api_key: str, host: str, port: int) -> ProxyClient:
        ...
        return cls(TunnelTransport(org, db, api_key, host=host, port=port), org, db)
    ...
    def _new_sink(self) -> PartSink:
        ...
        return R2PartSink(self._org, self._db)
